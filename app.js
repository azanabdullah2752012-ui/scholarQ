// ScholarQ - Academy OS (V108 Bento Edition)
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  supabaseClient.auth.onAuthStateChange(async (event, s) => {
    if (event === 'SIGNED_IN' || event === 'SIGNED_OUT') await handleSessionUpdate(s);
  });
...
// BENTO RENDERING ENGINE (V108)
function renderHome() {
  const view = document.getElementById('view-home');
  view.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:end; margin-bottom:32px;">
      <div><h1 style="margin-bottom:8px;">Academy Hub</h1><p style="color:var(--text-secondary);">Global academic activity and peer solutions.</p></div>
      <button onclick="navigateTo('ask')" class="btn-primary"><i data-lucide="plus"></i> New Doubt</button>
    </div>
    <div class="dashboard-grid">
      <div id="hub-main">
        <div style="display:flex; gap:12px; margin-bottom:24px; overflow-x:auto; padding-bottom:8px;" id="hub-filters"></div>
        <div id="hub-questions"></div>
      </div>
      <div id="hub-widgets">
        <div id="mastery-container" class="mastery-widget"></div>
        <div class="glass-card" style="margin-top:24px; padding:20px;">
          <h3 style="font-size:0.9rem; margin-bottom:12px;">Academy Pulse</h3>
          <div style="font-size:0.75rem; color:var(--text-secondary); display:flex; flex-direction:column; gap:8px;">
            ...
            <div style="display:flex; justify-content:space-between;"><span>Active Users</span><span>124</span></div>
            <div style="display:flex; justify-content:space-between;"><span>Newcomer Hope</span><span style="color:var(--p-500);">HIGH</span></div>
          </div>
        </div>
      </div>
    </div>
...
function renderFilters() {
  const filt = document.getElementById('hub-filters');
  ['All', 'Math', 'Science', 'History', 'Comp Sci'].forEach(s => {
    const b = document.createElement('button');
    b.className = 'glass-card';
    b.style.cssText = `font-size:0.75rem; padding:8px 16px; border-radius:10px; ${currentFilter === s ? 'background: var(--p-500); color: white; border-color:var(--p-500);' : ''}`;
...
function renderHubQuestions() {
  ...
  const displayList = [...list.filter(q => q.boosted), ...list.filter(q => !q.boosted).slice(0, 5).sort(() => Math.random() - 0.5), ...list.filter(q => !q.boosted).slice(5)];
  cont.innerHTML = `<div style="display:grid; grid-template-columns:1fr; gap:16px;">
    ${displayList.map(q => `<div class="glass-card" style="padding:24px; cursor:pointer; border-left:${q.boosted?'4px solid var(--p-500)':'1px solid var(--border)'};" onclick="navigateTo('question', '${q.id}')">
      ...
      <h3 style="font-size:1.1rem; line-height:1.4;">${escapeHTML(q.title)}</h3>
      <div style="display:flex; justify-content:space-between; align-items:center; margin-top:20px; font-size:0.75rem; color:var(--text-secondary);">
        ...
        <span style="font-weight:700;"><i data-lucide="message-square" style="width:14px; margin-right:4px;"></i>${q.answers?.length || 0}</span>
      </div>
    </div>`).join('')}
  </div>`;
}
...
function renderMicroTasks() {
  ...
  if (profile.last_challenge_date === today) {
    cont.innerHTML = `<h3 style="font-size:0.9rem;">Daily Mastery</h3><p style="font-size:0.75rem; opacity:0.6;">✅ Complete. Check back tomorrow for new concepts.</p>`;
    return;
  }
  ...
  cont.innerHTML = `
    <div style="font-size:0.7rem; font-weight:800; color:var(--p-500); margin-bottom:8px;">${isHardMode?'🔥 ELITE':'DAILY'} MASTERY ${gauntletStep+1}/3</div>
    <p style="font-weight:700; font-size:1rem; margin-bottom:16px;">${c.q}</p>
    <div style="display:flex; gap:8px;">
      <button onclick="handleGauntlet(true)" class="mastery-btn">${c.a}</button>
      <button onclick="handleGauntlet(false)" class="mastery-btn">${c.b}</button>
    </div>
  `;
}
