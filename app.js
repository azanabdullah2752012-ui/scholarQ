// ScholarQ - Institutional Stability OS (V107.1)
...
async function handleSessionUpdate(newS) {
  ...
  if (session) {
    ...
    // Only redirect to auth if we aren't already there and not initializing
...
function navigateTo(view, param = null) {
  // SESSION GUARD: Don't allow navigation to protected views without session
  if (!session && view !== 'auth' && !isInitializing) view = 'auth';
...
async function renderMarketplace() { const cont = document.getElementById('market-list'); cont.innerHTML = '<div style="padding:40px; text-align:center; opacity:0.5;">Loading Academy Resources...</div>'; const { data: allItems } = await supabaseClient.from('marketplace_items').select('*'); const { data: myP } = await supabaseClient.from('purchases').select('item_id').eq('user_id', session.user.id); const pIds = (myP || []).map(p => p.item_id); if (allItems) { const legends = allItems.filter(i => (i.downloads || 0) > 20).sort((a,b) => b.downloads - a.downloads), rising = allItems.filter(i => (i.downloads || 0) <= 20).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)); const displayList = [...rising.slice(0, 2), ...legends, ...rising.slice(2)]; cont.innerHTML = `<div style="display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:24px; width:100%;">${displayList.map(it => { const isB = pIds.includes(it.id) || it.seller_id === session.user.id, downloads = it.downloads || 0, isNew = (Date.now() - new Date(it.created_at).getTime()) < 172800000; return `<div class="glass-card" style="padding:24px; display:flex; flex-direction:column; justify-content:space-between;"><div><div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:16px;"><span class="badge" style="background:none; border:1px solid var(--border);">${isNew ? 'RISING' : 'TRUSTED'}</span><strong style="color:var(--p-500); font-size:1rem;">${it.price} PTS</strong></div><h3 style="margin-bottom:8px;">${escapeHTML(it.title)}</h3><p style="font-size:0.8rem; margin-bottom:16px;">${it.seller_name}</p></div>${isB ? `<a href="${it.link}" target="_blank" class="btn-primary" style="text-decoration:none; justify-content:center; padding:10px;">View Access</a>` : `<button onclick="handleBuyItem('${it.id}', ${it.price}, '${it.seller_id}')" class="btn-primary" style="justify-content:center; padding:10px;">Unlock Resource</button>`}</div>`; }).join('')}</div>`; } lucide.createIcons(); }
